App = {
  ...
  initContracts: function() {
    $.getJSON("parinTokenSale.json", function(parinTokenSale) {
      ...
      $.getJSON("parin.json", function(parin) {
        App.contracts.parin = TruffleContract(parin);
        App.contracts.parin.setProvider(App.web3Provider);
        App.contracts.parin.deployed().then(function(parin) {
          console.log("parin Token Address:", parin.address);
        });
  ...
  render: function() {
    ...
    App.contracts.parinTokenSale.deployed().then(function(instance) {
      ...
      // Load token contract
      App.contracts.parin.deployed().then(function(instance) {
        parinTokenInstance = instance;
        return parinTokenInstance.balanceOf(App.account);
      }).then(function(balance) {
        $('.parin-balance').html(balance.toNumber());
        App.loading = false;
        loader.hide();
        content.show();
      })
